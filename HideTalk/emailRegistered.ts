...
//? already Uploaded
exports.emailRegistered = functions.region('europe-west1').https.onCall(async (data, context) => {
  // const supUid = data.uid;
  const email = data.email;
  const sfRef = db.collection('Users').doc('{user}');
  const collections = await sfRef.listCollections();
  let emailFound = false;
  try {
    collections.forEach(async (collection) => {
      let docu = await collection.doc().get();
      console.log(docu.data.toString());
      if (docu.data()?.userEmail == email) {
        emailFound = true;
      } else {
