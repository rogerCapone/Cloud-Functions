  ...
  return db.doc('allUsers/usersDoc').get().then((allUsersDocu)=>{
    ...
    for (let index = 0; index < allUsers.length; index++) {
      if(allUsers[index].nickName.toString().toUpperCase().includes(userToFind.toUpperCase())){
        ...
        usersMatching.push({img:allUsers[index].img, nickName:allUsers[index].nickName ,uid: allUsers[index].uid});
        console.log(`${allUsers[index].nickName} inserted to the ARRAY`);
      }}
