...
export const getUsersPendingToAccept = functions.https.onCall(async (data, context) => {
      ...
      for (let index = 0; index < pdtList.length; index++) {
        usersData?.userList.forEach((user:any)=>{
          if(pdtList[index] == user.uid){
            finalArray.push({img: user.img, nickName: user.nickName, uid: user.uid, frase: ''});
          // }else{
          //   console.log(user);
          }
        })
      }
      // resp.send(finalArray);
    }).then(()=>{
