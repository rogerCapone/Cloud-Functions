...
exports.getFirebaseAirport = functions.https.onRequest(async (req,resp)=>{

  const airport = req.query.airport;
  const airportDoc = req.query.firebase;
    try{
      const airDoc =  await db.collection('AirData').doc(`${airportDoc}`).get();
        if(airDoc.exists){
          const json = airDoc.data();
          if(json!==undefined){
            Object.keys(json).forEach(async(ele)=>{
              if(ele.toLowerCase() === airport.toLowerCase() && json!==undefined){
                //podriem afegir location & web
