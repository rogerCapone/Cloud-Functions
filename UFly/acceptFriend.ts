...
export const acceptFriend = functions.https.onCall(async (data, context) => {
  ...
  const uid = data.uid;                        //req.query.uid; //Aquest es el user que accepta la solicitud
  const friendAccepted = data.newFriend;                      //req.query.newFriend; //Aquest es el user que es acceptat

  return db.doc(`users/${uid}`).update({
    pendingToAccept: admin.firestore.FieldValue.arrayRemove(friendAccepted),
    ...
    return db.doc(`users/${friendAccepted}`).update({
      ...
      console.log(`Now you allowed ${friendAccepted} to be your friend, you are: ${uid}`);
      return true;
    })
  }).catch(console.error);
})
