...
exports.decideToDeleteTopicSubscription = functions.firestore.document('subscriptions/{subscription}').onUpdate((change, context)=>{
  ...
  if(oldToken.length == 1 && oldUid.length == 1 && newToken.length == 0 && newUid.length == 0 ){
    ...
    console.log(oldToken.length, oldUid.length, newToken.length, newUid.length)
    return;
  }
